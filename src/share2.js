...
function visibleLinkedFiles(text=''){const refs=[],re=/(?:sandbox:)?\/mnt\/data\/[^\s)\]}'\"<>]+/g;for(const m of String(text).matchAll(re)){const path=m[0].replace(/[.,;:!?]+$/,''),kind=kindFrom(path,'','file');if(kind)refs.push({id:path.replace(/^sandbox:/,''),kind,name:path});}const seen=new Map();for(const r of refs)if(!seen.has(r.id))seen.set(r.id,r);return[...seen.values()];}
